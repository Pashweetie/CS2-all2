...
public class Test{
	public static void main(String[] args) {
			// // LinkedClass<Countries> country = returnCountries(importCount());
			// LinkedClass<Countries> countriboi= returnCountries(populateCountryArray());
			// LinkedClass<Countries> nearly = returnBoth(populateCountryArray(),importBord(),"Germany");
		...
		LinkedClass<Countries> countries = populateCountryArray("input.txt");
        String name =greet();
        String querie = greet1();
        LinkedClass<Countries> setTo=null;
        // System.out.println(	querie);
        if(querie.equals("pop")){
            setTo=returnCountries(populateCountryArray("input.txt"));
        ...
        if(querie.equals("bord")){
            Borders setToo=returnBord(importBord("borders.txt"),name);
            System.out.println(	setToo);
            System.out.println(	returnBord(importBord("borders.txt"),name));
            // System.out.println(setToo.getBorderName());
            setToo.printBorders();
            System.out.println();
        }
        if(querie.equals("both")){
            setTo=returnBoth(populateCountryArray("input.txt"),importBord("borders.txt"),name);
            for(int i=0;i<setTo.length();i++){
                System.out.print(setTo.get(i).getName());
            }
            System.out.println();
     ...
     public static Borders returnBord(LinkedClass<Borders>jimbo, String theName){
        LinkedClass<Borders> returnArray=new LinkedClass<Borders>();
  ...
  //       return returnArray;
  //  }
    public static LinkedClass<Countries> returnCountries(LinkedClass<Countries> jimbo){
        LinkedClass<Countries> returnArray = new LinkedClass<Countries>();
        for (int i=0;i<jimbo.length();i++) {
    ...
    public static LinkedClass<Countries> returnBoth(LinkedClass<Countries> jimbo,LinkedClass<Borders> reee,String theName){
        LinkedClass<Countries> countries =returnCountries(jimbo);
